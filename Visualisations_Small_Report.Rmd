...
Yes, this makes sense too. The number of tips in the phylogeny is exactly the number of rows in the data frame. It's seems that the column that matches them to each toher is ALLMB_OrigSpecies. Check this: 
...
I will randomly selected 75 tips/rows to higlight some combinations of options. The reason I am not immediately going into the full 350k is that rendering a figure of that size can actually take a while, so I want to have a bit of an idea first of where we are heading. A few directions follow. 
...
This of course compresses the really big species numbers, and also make immediately obvious what species don't have any data (i.e. the ones without bars). In that sense, it basically tells us the same as the inner ring of Figure 1, but also a bit more. We can improve it even further (Figure 5):
...
I think that at this stage, the most productive thing is to think a bit deeper about what we want the figure /figures to show, and ask how we can best approach this from there. What is the specific message of the figure in the manuscript. How much space do we have? One big figure? A few big figures? One big and a few small? 

I think it's best, Jens, if you think about this a bit more. We might even have a Skype-chat. If you send me a draft of the ms (do you have it already?), I could also think along those lines. For now, my two cents are:

- A version of the first figure, with as main message the simple patterns of presence / absence, potentially with coloured branches indicating either number of traits, specific traits of interest or some form of taxonomy is likely the key most interesting contribution.  
...
- Potential second addition: something with the six variables of interest, either individually, or as a single (binary variable). 
